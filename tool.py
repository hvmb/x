...
os.system(f"title boost tool by discord.gg/serverboost")
def getchecksum():
...
keyauthapp = api(
    name = "boost tool",
    ownerid = "GmscjReg1C",
    secret = "bc1a2095d1ed07bd0edd4e431fc976b0933d675778243fbb7f362c3bb273a491",
    version = "1.0",
...
def Auth():
        print(f"""{Style.BRIGHT}{Fore.RED}[!]{Style.BRIGHT}{Fore.WHITE} [Login]
{Fore.RED}[2] {Style.BRIGHT}{Fore.WHITE}[Sign Up]
""")


 
        ans = input(f"{Fore.RED}[!]{Fore.WHITE} Option 1 or 2:{Fore.WHITE} ")
        if ans == "1":
            user = input(f'{Style.BRIGHT}{Fore.GREEN}[+] {Fore.WHITE}Username: ')
            password = input(f'{Style.BRIGHT}{Fore.GREEN}[+] {Fore.WHITE}Password: ')
            keyauthapp.login(user, password)
            time.sleep(.1)
        elif ans == "2":
            user = input(f'{Fore.YELLOW}[!] {Fore.WHITE}Username ')
            password = input(f'{Fore.YELLOW}[!] {Fore.WHITE}Password ')
            license = input(f'{Fore.YELLOW}[!] {Fore.WHITE}License ')
...
def sprint(message, type):
    if type == True:
        print(f"{Style.BRIGHT}{Fore.GREEN}[+]{Style.BRIGHT}{Fore.YELLOW} {message}{Fore.RESET}{Style.RESET_ALL}")
    if type == False:
    ...
    try:
        for i in range(10):
            ...
            if response.status_code == 429:
                sprint(f"[{thread}] rate limited Sleeping for 10 secs", False)
                time.sleep(10)
            ...
            if len(boost_data.json()) != 0:
                ...
                if join_outcome:
                    ...
                    if config["change_server_nick"]:
                        ...
                        if changed:
                            sprint(f"[{thread}] {token} {nick}", True)
                        else:
                            sprint(f"[{thread}] error renameing {token}", False)
                else:
                    sprint(f"[-] [{thread}] {token} error joining", False)
                    open("error_joining.txt", "a").write(f"\n{token}")
                    remove(token, filename)
                    variables.failed_tokens.append(token)
            else:
                remove(token, filename)
                sprint(f"[!] [{thread}] {token} has no nitro", False)
                variables.failed_tokens.append(token)
...
print(f'''{Style.BRIGHT}{Fore.MAGENTA}
                        ...
                        _                 _   _                             _           
                       | |__  ___  ___ __| |_(_)_ _  __ _   ___ ___ _ ___ _(_)__ ___ ___
                       | '_ \/ _ \/ _ (_-<  _| | ' \/ _` | (_-</ -_) '_\ V / / _/ -_|_-<
                       |_.__/\___/\___/__/\__|_|_||_\__, | /__/\___|_|  \_/|_\__\___/__/
                                                    |___/                               
                                   _                 _     _            _ 
                                  | |__  ___  ___ __| |_  | |_ ___  ___| |
                                  | '_ \/ _ \/ _ (_-<  _| |  _/ _ \/ _ \ |
                                  |_.__/\___/\___/__/\__|  \__\___/\___/_|
''')
